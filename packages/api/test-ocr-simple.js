// Simple CommonJS test for OCR (no TypeScript compilation needed)
const path = require('path');

// We'll compile the TypeScript first, then run this
async function test() {
  // Import the compiled JS
  const { parseMatricCertificate } = await import('./dist/utils/ocr.js');
  ...
  process.exit(apsMatch && subjectCount === 7 ? 0 : 1);
}
