  ...
  try {
    ...
    const payload: ApplicationPayload = {
      ...
      documents: {
        ...
        matricCertificate: student.documents.find((d) => d.type === 'matric_certificate')
          ?.storageKey,
        proofOfResidence: student.documents.find((d) => d.type === 'proof_of_residence')
          ?.storageKey,
      },
    };
...
export async function submitMultipleApplications(
  ...
  results.forEach((result, index) => {
    ...
    if (result.status === 'fulfilled') {
      ...
      logger.error(
        { applicationId, errorMessage: err?.message, errorStack: err?.stack },
        'Application submission failed'
      );
    }
  });
